...
export const Comment = ({ comment }: { comment: AppBskyFeedDefs.ThreadViewPost }) => {
	...
	const avatarClassName = 'h-6 w-6 shrink-0 rounded-full bg-hp3'
		...
		if (segment.isLink() && segment.link) {
			richText.push(
				<Link
					...
					className="text-hp3 decoration-hp4 decoration-dashed hover:underline underline-offset-2"
				>
					{segment.text}
				</Link>
			)
	...
	return (
		<div className="my-4 text-base">
			...
			{comment.replies && comment.replies.length > 0 && (
				<div className="pl-2 ml-2 border-l border-hp2">
...
export const Actions = ({ post, url }: { post: AppBskyFeedDefs.PostView; url: string }) => (
	<div className="flex flex-row items-center justify-between w-full max-w-44 text-white/60">
		...
		<Link href={url} target="_blank" rel="noreferrer noopener" className="flex flex-row items-center gap-1.5 hover:text-hp3">
