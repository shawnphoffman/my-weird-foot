...
export default function PostRow({ post }: Props) {
	...
	return (
		<a href={`/updates/${slug}`} className="w-full mx-auto text-left text-white md:w-full group max-md:max-w-96 ">
			<div className="flex flex-col items-center justify-between gap-2 px-0 py-4 transition-all md:py-2 md:px-4 md:flex-row md:gap-4 hover:bg-boba-grayn/10 rounded-xl">
				{mainImage && (
					<Image
				...
				<div className="flex flex-row w-full gap-2">
					...
					<div className="flex flex-col items-start justify-center flex-1 w-full gap-1">
						<h2 className="text-xl font-bold leading-tight transition-colors group-hover:text-white text-boba-green">{title}</h2>
						{/* <span className="md:hidden"> */}
						<PostDate dateString={publishedAt} />
						{/* </span> */}
						<span className="text-sm leading-tight text-boba-green">{post.excerpt}</span>
					</div>
				</div>
