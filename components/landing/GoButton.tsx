...
export default function GoButton({ i }) {
	...
	const bhClasses = classNames(i === 1 ? 'visible' : 'invisible h-0')
	const hpClasses = classNames(i !== 1 ? 'visible' : 'invisible h-0')
	...
	return (
		<div className={`flex justify-center m-4 max-[600px]:mb-24 ${containerClasses}`}>
			...
			>
				...
				<Image className={`w-full h-full max-w-[400px] ${bhClasses}`} alt="" src={bh} width={400} draggable="false" />
				<Image className={`w-full h-full max-w-[400px] ${hpClasses}`} alt="" src={hp} width={400} draggable="false" />
			</Link>
		</div>
	)
}
